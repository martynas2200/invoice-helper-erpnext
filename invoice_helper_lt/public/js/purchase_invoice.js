frappe.ui.form.on("Purchase Invoice", {
	async refresh(frm) {
		const button = frm.add_custom_button(__("Prefill from Pending Document"), () => prefill_from_pending_dialog(frm));
		if (frm.is_new()) {
...
function prefill_from_pending_dialog(frm) {
	const d = new frappe.ui.Dialog({
		...
		fields: [
			{
				...
				fieldtype: "Link",
				get_query: () => ({ filters: { type: "Purchase" }, order_by: "`tabPending Document`.`id` desc" }),
			...
			},
			{
				...
				description: __("If available, Bill No, Posting Date, Due Date, Supplier from Pending Document"),
			},
		],
		...
		primary_action: async (values) => {
			...
			d.hide();
			await prefill_from_pending(frm, values.pending, values.prefill_type, values.should_header_be_prefilled);
		},
	});
	d.show();
}

async function prefill_from_pending(frm, pendingName, prefillType = "textract_tables", shouldHeaderBePrefilled = true) {
	...
	if (shouldHeaderBePrefilled) {
		...
		if (!frm.doc.due_date && pd.due_date) {
			...
			const postingDate = frm.doc.posting_date ? frappe.datetime.str_to_obj(frm.doc.posting_date) : new Date();
	...
	if (barcodes.length) {
		try {
			const res = await frappe.call({
				method: "invoice_helper_lt.api.get_item_codes_for_barcodes",
	...
	const typeLabel = prefillType === "local_tables" ? __("Local extraction") : prefillType === "textract_tables" ? __("Amazon Textract") : __("Regular expression: Barcodes");
	frappe.show_alert({
		message: __("Used {0}. Items - Matched: {1}, Unmatched: {2}", [typeLabel, matched, unmatched]),
...
function detectColumnType(values, headerText = "") {
	let barcodeScore = 0, quantityScore = 0, priceScore = 0;
	...
	else if ((headerText.toLowerCase().includes("price") || headerText.toLowerCase().includes("kaina")) && priceScore > 0) {
		return "price";
	}
	else if ((headerText.toLowerCase().includes("quantity") || headerText.toLowerCase().includes("kiekis"))&& quantityScore > 0) {
		return "quantity";
	}
...
async function selectTableAndColumnsFromTextractData(pendingDoc, docType, variable = "tables") {
	return new Promise((resolve) => {
		...
		if (!tables || tables.length === 0) {
			frappe.show_alert({ message: __("No tables found in the document"), indicator: "red" });
			resolve([]);
			return;
		}
		...
		if (tables.length > 1) {
			...
			tables.forEach((table, idx) => {
				const button = $(`<button class="btn btn-default" style="margin: 5px; display: block; width: 100%; padding: 10px;">
					${__("Table")} ${idx + 1} (${table.rows.length} ${__("rows")}, ${table.rows[0]?.length || 0} ${__("columns")}) - ${__("Confidence")}: ${(table.confidence || 0).toFixed(1)}%
				</button>`);
				button.click(() => {
					selectedTable = table;
					d.hide();
...
function showColumnMappingDialog(table, resolve) {
	...
	const d = new frappe.ui.Dialog({
		...
		primary_action: (values) => {
			...
			const duplicates = selectedTypes.filter((item, index) => selectedTypes.indexOf(item) !== index);
			if (duplicates.length > 0) {
				frappe.show_alert({ message: __("Please ensure each column type is selected only once."), indicator: "red" });
				return;
			}
	...
	const preview = $(`<table class="table table-bordered" style="font-size: 11px; max-height: 200px; overflow-y: auto;">
		<tbody></tbody>
	</table>`);
...
function extractMappedRows(table, columnMapping) {
	...
	const firstRowText = (table.rows[0] || [])
		...
		.join(" ");
	if (!/^(nr|item|barcode|qty|quantity|price|amount|rate|total|kaina|kiekis|suma|barkodas|kodas)/i.test(firstRowText)) {
		hasHeader = false;
	}
