	...
	def extract_from_document(self, file_path: str) -> dict[str, any]:
		...
		try:
			...
			if file_path.startswith("s3://"):
				...
				response = self.client.detect_document_text(Document={"Bytes": file_content})
			return self._parse_response(response)
		except Exception as e:
			frappe.logger().error(f"Textract extraction error: {e!s}", "Textract Extraction Error")
			frappe.log_error(f"Textract extraction error: {e!s}", "Textract Extraction Error")
			raise
			...
			if file_path.startswith("s3://"):
				...
				if file_content[:4] != b"%PDF":
					frappe.logger().warning(
						f"Textract: File may not be a valid PDF! First 4 bytes: {file_content[:4]}"
					)

				doc_uri = {"Bytes": file_content}

			frappe.logger().info("Textract: Starting AnalyzeDocument call with FeatureTypes=['TABLES']")
			response = self.client.analyze_document(Document=doc_uri, FeatureTypes=["TABLES"])
			return self._parse_tables_response(response)
		except Exception as e:
			frappe.logger().error(f"Textract table extraction error: {e!s}", "Textract Extraction Error")
			frappe.log_error(f"Textract table extraction error: {e!s}", "Textract Extraction Error")
			raise
